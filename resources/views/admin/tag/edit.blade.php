                ...
                <div class="col-12">
                    <form action="{{ route('admin.tag.update', $tag->id) }}" method="POST" class="w-25">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" name="title" placeholder="Название тэга" value="{{ $category->title }}">
                            @error('title')
                            <div class="text-danger">Это поле необходимо заполнить!</div>
                            @enderror
                        </div>
                        <input type="submit" value="Обновить" class="btn btn-block bg-gradient-primary btn-lg"></input>
                    </form>
                </div>
            </div>
            <!-- /.row -->
